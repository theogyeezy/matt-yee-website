import React, { useEffect } from 'react';
import { motion } from 'framer-motion';
import styled from 'styled-components';
import { Helmet } from 'react-helmet';

const ConsultingContainer = styled.div`
  min-height: 100vh;
  padding-top: 80px;
  background: #0a0a0a;
`;

const HeroSection = styled.section`
  padding: 4rem 0;
  background: linear-gradient(135deg, rgba(102, 126, 234, 0.1) 0%, rgba(118, 75, 162, 0.1) 100%);
`;

const Container = styled.div`
  max-width: 900px;
  margin: 0 auto;
  padding: 0 2rem;
`;

const Title = styled(motion.h1)`
  font-size: clamp(2rem, 5vw, 3rem);
  text-align: center;
  margin-bottom: 2rem;
  background: linear-gradient(135deg, #667eea 0%, #764ba2 100%);
  -webkit-background-clip: text;
  -webkit-text-fill-color: transparent;
  font-family: 'Orbitron', monospace;
`;

const Subtitle = styled(motion.p)`
  font-size: clamp(1.2rem, 3vw, 1.5rem);
  text-align: center;
  color: #cccccc;
  margin-bottom: 3rem;
  font-style: italic;
  line-height: 1.6;
`;

const ContentSection = styled.section`
  padding: 4rem 0;
`;

const ServiceGrid = styled.div`
  display: grid;
  grid-template-columns: repeat(auto-fit, minmax(300px, 1fr));
  gap: 2rem;
  margin: 3rem 0;
`;

const ServiceCard = styled(motion.div)`
  background: #111111;
  padding: 2rem;
  border-radius: 10px;
  border: 1px solid #222222;
  transition: all 0.3s ease;
  
  &:hover {
    transform: translateY(-3px);
    border-color: #667eea;
    box-shadow: 0 8px 32px rgba(102, 126, 234, 0.1);
  }
`;

const ServiceIcon = styled.div`
  font-size: 2.5rem;
  margin-bottom: 1rem;
`;

const ServiceTitle = styled.h3`
  color: #ffffff;
  font-size: 1.3rem;
  margin-bottom: 1rem;
  font-family: 'Orbitron', monospace;
`;

const ServiceDescription = styled.p`
  color: #cccccc;
  line-height: 1.6;
  font-size: 1rem;
  margin-bottom: 1rem;
`;

const ServiceHighlights = styled.ul`
  color: #999999;
  font-size: 0.9rem;
  
  li {
    margin-bottom: 0.5rem;
    padding-left: 1rem;
    position: relative;
    
    &:before {
      content: "▸";
      position: absolute;
      left: 0;
      color: #667eea;
    }
  }
`;

const CTASection = styled.section`
  padding: 4rem 0;
  background: #111111;
  text-align: center;
`;

const CTAText = styled(motion.p)`
  color: #cccccc;
  font-size: 1.2rem;
  margin-bottom: 2rem;
  line-height: 1.6;
`;

const CTAButton = styled(motion.a)`
  display: inline-block;
  background: linear-gradient(135deg, #667eea 0%, #764ba2 100%);
  color: white;
  padding: 1rem 2rem;
  border-radius: 5px;
  text-decoration: none;
  font-weight: 600;
  font-size: 1.1rem;
  transition: all 0.3s ease;
  
  &:hover {
    transform: translateY(-2px);
    box-shadow: 0 8px 32px rgba(102, 126, 234, 0.3);
  }
`;

const Consulting = () => {
  useEffect(() => {
    window.scrollTo(0, 0);
  }, []);

  const containerVariants = {
    hidden: { opacity: 0 },
    visible: {
      opacity: 1,
      transition: {
        staggerChildren: 0.1,
      },
    },
  };

  const itemVariants = {
    hidden: { y: 20, opacity: 0 },
    visible: {
      y: 0,
      opacity: 1,
      transition: {
        duration: 0.5,
      },
    },
  };

  return (
    <>
      <Helmet>
        <title>Consulting Services - Matt Yee</title>
        <meta name="description" content="Expert AI/ML consulting, startup strategy, and innovation leadership from Matt Yee. Transform wild ideas into breakthrough solutions." />
        <meta property="og:title" content="AI/ML Consulting & Innovation Strategy - Matt Yee" />
        <meta property="og:description" content="Consulting services in AI/ML implementation, startup strategy, and innovation leadership from Austin, Texas." />
        <meta name="keywords" content="AI ML consulting, artificial intelligence consultant, startup strategy, innovation leadership, Austin Texas, tech consulting" />
      </Helmet>

      <ConsultingContainer>
        <HeroSection>
          <Container>
            <Title
              initial={{ opacity: 0, y: -20 }}
              animate={{ opacity: 1, y: 0 }}
              transition={{ duration: 0.5 }}
            >
              Consulting Services
            </Title>
            <Subtitle
              initial={{ opacity: 0 }}
              animate={{ opacity: 1 }}
              transition={{ duration: 0.5, delay: 0.2 }}
            >
              Where wild ideas turn into breakthrough solutions
            </Subtitle>
          </Container>
        </HeroSection>

        <ContentSection>
          <Container>
            <motion.div
              variants={containerVariants}
              initial="hidden"
              animate="visible"
            >
              <motion.p variants={itemVariants} style={{ color: '#cccccc', fontSize: '1.1rem', lineHeight: '1.8', marginBottom: '3rem', textAlign: 'center' }}>
                I help companies and startups navigate the intersection of <strong style={{ color: '#ffffff' }}>cutting-edge AI/ML</strong>, 
                <strong style={{ color: '#ffffff' }}> strategic innovation</strong>, and <strong style={{ color: '#ffffff' }}>scalable execution</strong>. 
                From Austin, Texas, I bring think tank-level strategy with real-world implementation experience.
              </motion.p>

              <ServiceGrid>
                <ServiceCard
                  variants={itemVariants}
                  whileHover={{ scale: 1.02 }}
                  initial={{ opacity: 0, y: 20 }}
                  whileInView={{ opacity: 1, y: 0 }}
                  viewport={{ once: true }}
                >
                  <ServiceIcon>🧠</ServiceIcon>
                  <ServiceTitle>AI/ML Strategy & Implementation</ServiceTitle>
                  <ServiceDescription>
                    Transform your business with custom AI/ML solutions that actually deliver ROI. 
                    From strategy to deployment, I help companies build AI that works.
                  </ServiceDescription>
                  <ServiceHighlights>
                    <li>AI strategy development & roadmapping</li>
                    <li>Machine learning architecture design</li>
                    <li>Team building & technical leadership</li>
                    <li>Proof of concept to production scaling</li>
                  </ServiceHighlights>
                </ServiceCard>

                <ServiceCard
                  variants={itemVariants}
                  whileHover={{ scale: 1.02 }}
                  initial={{ opacity: 0, y: 20 }}
                  whileInView={{ opacity: 1, y: 0 }}
                  viewport={{ once: true }}
                  transition={{ delay: 0.1 }}
                >
                  <ServiceIcon>🚀</ServiceIcon>
                  <ServiceTitle>Startup Strategy & Scaling</ServiceTitle>
                  <ServiceDescription>
                    Navigate the startup journey with battle-tested strategies for building, 
                    scaling, and avoiding the pitfalls that kill great ideas.
                  </ServiceDescription>
                  <ServiceHighlights>
                    <li>Go-to-market strategy development</li>
                    <li>Product-market fit validation</li>
                    <li>Fundraising strategy & pitch optimization</li>
                    <li>Team building & organizational design</li>
                  </ServiceHighlights>
                </ServiceCard>

                <ServiceCard
                  variants={itemVariants}
                  whileHover={{ scale: 1.02 }}
                  initial={{ opacity: 0, y: 20 }}
                  whileInView={{ opacity: 1, y: 0 }}
                  viewport={{ once: true }}
                  transition={{ delay: 0.2 }}
                >
                  <ServiceIcon>💡</ServiceIcon>
                  <ServiceTitle>Innovation Leadership</ServiceTitle>
                  <ServiceDescription>
                    Build internal innovation capabilities that consistently turn wild ideas 
                    into market-changing products and services.
                  </ServiceDescription>
                  <ServiceHighlights>
                    <li>Innovation lab setup & operations</li>
                    <li>Corporate venture building</li>
                    <li>Design thinking & rapid prototyping</li>
                    <li>Innovation culture development</li>
                  </ServiceHighlights>
                </ServiceCard>

                <ServiceCard
                  variants={itemVariants}
                  whileHover={{ scale: 1.02 }}
                  initial={{ opacity: 0, y: 20 }}
                  whileInView={{ opacity: 1, y: 0 }}
                  viewport={{ once: true }}
                  transition={{ delay: 0.3 }}
                >
                  <ServiceIcon>🎯</ServiceIcon>
                  <ServiceTitle>Executive Advisory</ServiceTitle>
                  <ServiceDescription>
                    Strategic guidance for executives navigating digital transformation, 
                    AI adoption, and building technology-driven competitive advantages.
                  </ServiceDescription>
                  <ServiceHighlights>
                    <li>Technology strategy & digital transformation</li>
                    <li>Board-level AI/ML briefings</li>
                    <li>Competitive intelligence & market analysis</li>
                    <li>Strategic partnership development</li>
                  </ServiceHighlights>
                </ServiceCard>
              </ServiceGrid>
            </motion.div>
          </Container>
        </ContentSection>

        <CTASection>
          <Container>
            <CTAText
              initial={{ opacity: 0 }}
              whileInView={{ opacity: 1 }}
              viewport={{ once: true }}
            >
              Ready to turn your wild ideas into breakthrough solutions?
            </CTAText>
            <CTAButton
              href="mailto:matt.sam.yee@gmail.com?subject=Consulting%20Inquiry"
              initial={{ opacity: 0, y: 20 }}
              whileInView={{ opacity: 1, y: 0 }}
              viewport={{ once: true }}
              transition={{ delay: 0.2 }}
            >
              Let's Talk Strategy
            </CTAButton>
          </Container>
        </CTASection>
      </ConsultingContainer>
    </>
  );
};

export default Consulting;